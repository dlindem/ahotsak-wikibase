...
print('\nLoading data...')
with open('D:/Ahotsak/wikibase/ETC/writeprogress.txt', 'r', encoding="utf-8") as file:
	...
	for entry in etcfile:
		...
		for form in entry['formak']:
			...
			for analisia in form['analisiak']:
				...
				for index in range(len(zatiketa)):
					if zatiketa[index] == "0":
						del zatiketa[index]
				...
				if form['forma'] in existing_forms:
					if len(existing_forms[form['forma']]) > 0:
					...
					formid = awb.newform(lid, form['forma'])
				# write ETC info
				...
				# if len(laburdurak) == 0:
				# 	print('Next to POS value, no analisi for this form to convert into gramgrp code.')
				# 	continue
				...
				if "+".join(gramgrp) in seen_grmgrp:
					formid = seen_grmgrp["+".join(gramgrp)]
